            ...
            while (true) {
                ...
                for (ConsumerRecord<String, HubEventAvro> record : records) {
                    ...
                    if (handler != null) {
                        ...
                        log.error("Не могу найти обработчик для события {}", payloadName);
                        throw new IllegalArgumentException("Не могу найти обработчик для события " + payloadName);
                    }
                }

                hubConsumer.commitSync();
                log.debug("Коммит смещений для топика хабов");
            }
        } catch (WakeupException ignored) {
            log.info("Получен сигнал завершения для HubEventProcessor");
        } catch (Exception e) {
